...
export function RoomSettingsPanel() {
  ...
  return <GameCard className="lobby-settings"><div className="lobby-section-heading"><GameIcon name="settings" size={18} /><h2>SET THE MOOD.</h2></div><p>{host ? "Your room. Your questionable plan." : "The host sets the room. You bring the trouble."}</p>
    ...
    <details className="lobby-settings-group"><summary>GAME <span>THE PLAN</span></summary><div><fieldset disabled={disabled || !host}><legend className="sr-only">Game settings, controlled by the host</legend>
      ...
      <label className="lobby-field">SECRET INTENSITY<select value={room.settings.chaos} onChange={(event) => setting("chaos", event.target.value)}>{CHAOS_LEVELS.map((chaos) => <option key={chaos} value={chaos}>{chaos.toUpperCase()}</option>)}</select><span>{CHAOS_DESCRIPTIONS[room.settings.chaos]}</span></label>
    </fieldset><p className="settings-note">Secret Intensity changes private objective difficulty, risk, conflicts, and interaction. It does not change the deck, Effect strength, timers, or scoring.</p></div></details>
