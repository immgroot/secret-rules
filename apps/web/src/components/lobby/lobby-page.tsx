...
export function LobbyPage({ code }: { code: string }) {
  ...
  return <div className="site-frame lobby-shell" data-reduce-motion={reducedMotion}>
    ...
    <main className="content-width" id="lobby-main" tabIndex={-1}>
      {matching && room ? <>
        {room.roomNotice?.kind === "match_abandoned" && <section className="match-abandoned-notice" role="status" aria-labelledby="match-abandoned-title"><GameIcon name="warning" size={24} /><div><p className="eyebrow">MATCH ENDED</p><h2 id="match-abandoned-title">{room.roomNotice.message}</h2><p>The match was abandoned without a winner or bonus points. Your room is still here when the table is ready again.</p></div></section>}
